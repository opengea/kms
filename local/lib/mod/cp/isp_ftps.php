...
class isp_ftps extends mod {
	...
	var $readonly = array("login","vhost_id","shell","quota","inici");
	var $orderby = "id";
	var $notedit=array("vhost_id","vhost_id","quota","shell","home","vr_public_url","vr_ftp_host","type","created_at","updated_at","deleted_at");
	...
	var $can_export = false;
       ...
       //*=[ CONSTRUCTOR ]===========================================================*/
        ...
        function isp_ftps($client_account,$user_account,$dm,$dblinks) {
		...
		$dblink_cp=$dblinks['client'];
                ...
                if (!$result) die("[isp_ftps] error ".$select." ".mysql_error($result));
		...
		$result=mysql_query($sel,$dblink_cp);
                if (!$result) die('[isp_ftps] error executing '.$sel); 
                $vhost=mysql_fetch_array($result);
                if ($vhost['id']==""&&$_GET['app']!='sysadmin'&&$_GET['app']!='cp-admin') {
             	   print_r($vhost);die("[isp_ftps] error. Record not found. :".$vhost['id'].":<br>Query:".$sel);
                }
...
//echo $this->where;
       ...
       //         if ($_GET['_']=="i") $this->setComponent("cipher","password","plain"); //amb edit no, perque aixi la podem consultar
		...
		$row=$this->dbi->get_record("select count(*) as n from kms_isp_ftps where vhost_id=".$_GET['xid']." order by id desc limit 1");
		$n=$row['n']+1;
		if ($_GET['_']=='i')  $this->defvalue("login","v".$_GET['xid']."_u".$n);
	...
	function onPreInsert($post,$id) {
        ...
        function onInsert ($post,$id) {
                if ($post['login']=="root") return false;
                if ($_GET['xid']=="")  $this->_error("","xid param missing","fatal");
		...
		$server=$this->dbi->get_record("select hostname,ip from kms_isp_servers where id=".$vhost['webserver_id']); // ns4
                $command = "ssh -i /root/.ssh/id_rsa root@".$server['hostname']." '/usr/local/kms/mod/isp/setup/create_ftp.sh ".$vhost['name']." ".$post['login']." ".$ftp['password']."' >> /var/log/kms/kms.log";
//if ($_SERVER['REMOTE_ADDR']=='88.12.33.163') { echo $command;exit; }
	...
	function onDelete ($post,$id) {
		...
		if ($post['type']!="") $type=$post['type']; else $type="web_users";
		$command = "ssh -i /root/.ssh/id_rsa root@".$server['hostname']." 'rm -r /var/www/vhosts/".$vhost['name']."/{$type}/".$post['login']."' >> /var/log/kms/kms.log";
		$this->ssh_exec("localhost",$command,"JADF7320cSJdcj3750492x42dj244");
		$command = "ssh -i /root/.ssh/id_rsa root@".$server['hostname']." 'userdel ".$post['login']."' >> /var/log/kms/kms.log";
		$this->ssh_exec("localhost",$command,"JADF7320cSJdcj3750492x42dj244");
	}
	...
	function onUpdate ($post,$id) {
		...
		$command="ssh -i /root/.ssh/id_rsa root@".$server['hostname']." 'id -u ".$post['login']." &>/dev/null || useradd -p ".$post['password']." -s /bin/false -g intergrid -d ".$ftp['home']." -c \"KMS ftp user\" ".$post['login']."' >> /var/log/kms/kms.log";
//echo $command;exit;
		$this->ssh_exec("localhost",$command,"JADF7320cSJdcj3750492x42dj244");
