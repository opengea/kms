...
class isp_domains extends mod {
	...
	var $key	= "id";	
	var $fields = array("id","creation_date","expiration_date","status","auto_renew","dns_zone_id","sr_client","hosting_id");
...
//	var $excludeBrowser = array("name");
	...
	var $sortdir = "desc";
	var $hidden=array("zonec_fullname","zonec_organization","zonec_address","zonec_postalcode","zonec_city","zonec_country","zonec_state","zonec_phone","zonec_fax","zonec_email","sr_ownerc","sr_adminc","sr_techc","sr_zonec","vr_limits","vr_services","comments");
	var $notedit=array("zonec_fullname","zonec_organization","zonec_address","zonec_postalcode","zonec_city","zonec_country","zonec_state","zonec_phone","zonec_fax","zonec_email","sr_ownerc","sr_adminc","sr_techc","sr_zonec","hosting_id","expiration_date","dr_folder","client_id","sr_entity","sr_client","creation_date","updated_date","dns_zone_id","sr_contract","comments","vr_limits","vr_services");
	var $mandatory=array("ownerc_fullname","ownerc_address","ownerc_postalcode","ownerc_city","ownerc_phone","ownerc_fax","ownerc_email","ownerc_country","adminc_fullname","adminc_address","adminc_postalcode","adminc_city","adminc_phone","adminc_fax","adminc_email","adminc_country");
        /*=[ PERMISSIONS ]===========================================================*/
	...
	var $can_export = false;
       ...
       //*=[ CONSTRUCTOR ]===========================================================*/
        ...
        function isp_domains($client_account,$user_account,$dm,$dblinks) {
		if (!$silent) {
                $select="SELECT * FROM kms_isp_clients where sr_user=".$user_account['id'];
                $result=mysqli_query($dblinks['client'],$select);
                if (!$result) die("[isp_domains] mysql query error ".mysqli_error($result));
		...
		$this->setComponent("domain_name","name");
			...
			$this->can_edit=true;
			$this->notedit=array();//array("hosting_id","_folder","client_id","sr_entity","creation_date","updated_date","dns_zone_id","sr_contract");
			$this->readonly=array();//array("name","creation_date","updated_date","hosting_id","sr_entity");
			$this->fields = array("id","creation_date", "sr_client", "status", "expiration_date","auto_renew","dns_zone_id","hosting_id");
		...
		$this->setGroup("_KMS_ISP_DOMAINS_RENEW_OPTIONS",false,array("status","registration_date","expiration_date","auto_renew","renew_years","uthcode"));
                $this->setGroup("_KMS_ISP_DOMAINS_NAMESERVERS",true,array("nameserver1","nameserver2","nameserver3","nameserver4"));
		$this->setGroup("_KMS_ISP_DOMAINS_OWNERC",true,array("ownerc_fullname","ownerc_organization","ownerc_cif","ownerc_address","ownerc_postalcode","ownerc_city","ownerc_country","ownerc_state","ownerc_fax","ownerc_email","ownerc_phone"));
		...
		$this->humanize("updated_date","Last updated");
 ...
 //       $this->xvField("vr_services",array("sql"=>$xsql));
	...
	function onPreUpdate ($post,$id) {
		...
		if ($old_values['status']=="PENDING"&&($post['status']=="ACTIVE"||$post['status']=="LOCK")) {
			...
			$case=true;
			if ($isp_client['language']==null) $isp_client['language']="ca";
		...
		if ($modificat) {
			...
			$post['nameserver2']=trim($post['nameserver2']);
		        ...
		        curl_exec ($curlOb);
		        curl_close ($curlOb);
			if ($response['id']!="") $post['status']="PENDING";
			return $post;
	}
	}
	...
	function onUpdate ($post,$id) {
		...
		$this->dbi->update_record("kms_isp_domains",$post,"name='".$post['name']."'",$dblink_cp);
