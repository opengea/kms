...
class isp_dns extends mod {
	...
	var $can_search = true;
       ...
       //*=[ CONSTRUCTOR ]===========================================================*/
        ...
        function isp_dns($client_account,$user_account,$dm,$silent) {
...
/*		$sel ="SELECT * from kms_isp_domains WHERE name='".$vhost['name']."'";
		...
		if ($_GET['_']=="e"||$_GET['_']=="i") $this->setComponent("select","type",array("NS"=>"NS","A"=>"A","AAA"=>"AAA","CNAME"=>"CNAME","MX"=>"MX","PTR"=>"PTR","TXT"=>"TXT","SRV"=>"SRV"));
		...
		if ($_SERVER['REMOTE_ADDR']==TARTARUS_IP) $this->customButtons[1] = Array ("label"=>"Restaurar DKIM","url"=>"","ico"=>"reload_icon.gif","params"=>"action=rebuildDKIM","target"=>"_self","checkFunction"=>"","class"=>"");
...
//		$this->script("function updateF(t) { if (t=='NS')  $('#label_val').html(\""._KMS_ISP_DNS_RECS_NS_NAME."\"); }  else if (t=='A') { $('#label_val').html(\""._KMS_ISP_DNS_RECS_IP_ADDRESS."\"); } ");
		$this->script("function updateF(t) { $('textarea#val').css('height','25px');$('textarea#val').css('resize','none');if (t=='NS') { $('#label_host').html(\""._KMS_ISP_DNS_RECS_DOMAIN."\"); $('#label_val').html(\""._KMS_ISP_DNS_RECS_NS_NAME."\"); $('#comment_val').html('.'); $('#comment_host').html('.".$vhost['name'].".'); } else if (t=='A') { $('#label_host').html(\""._KMS_ISP_DNS_RECS_DOMAIN."\"); $('#label_val').html(\""._KMS_ISP_DNS_RECS_IP_ADDRESS."\"); $('#comment_val').html(''); $('#comment_host').html('.".$vhost['name'].".'); } else if (t=='CNAME') { $('#label_host').html(\""._KMS_ISP_DNS_RECS_DOMAIN."\"); $('#label_val').html(\""._KMS_ISP_DNS_RECS_CANONICAL_NAME."\"); $('#comment_val').html('.'); $('#comment_host').html('.".$vhost['name'].".'); } else if (t=='MX') { $('#label_host').html(\""._KMS_ISP_DNS_RECS_DOMAIN."\"); $('#label_val').html(\""._KMS_ISP_DNS_RECS_MAIL_SERVER."\"); $('#comment_val').html('.'); $('#comment_host').html('.".$vhost['name'].".'); } else if (t=='PTR') { $('#label_host').html(\""._KMS_ISP_DNS_RECS_IP_ADDRESS."\");  $('#label_val').html(\""._KMS_ISP_DNS_RECS_DOMAIN."\"); $('#val').val('".$vhost['name']."'); $('#comment_val').html(''); i=$('#host').val().indexOf('/'); if (i>0) { $('#host').val($('#host').val().substr(0,i) ); }    $('#comment_host').html(' / <select name=PTR_mask id=PTR_mask><option value=8>8</option><option value=16>16</option><option value=24 selected>24</option></select>'); }  else if (t=='TXT') {  $('textarea#val').css('height','200px'); $('#label_host').html(\""._KMS_ISP_DNS_RECS_DOMAIN."\"); $('#comment_host').html('.".$vhost['name'].".'); $('#label_val').html(\""._KMS_ISP_DNS_RECS_TXT_REGISTER."\"); $('#comment_val').html(''); } else if (t=='SRV') { $('#label_host').html(\""._KMS_ISP_DNS_RECS_SERVICE_PROTO."\"); $('#comment_host').html('.".$vhost['name'].".'); $('#label_val').html(\""._KMS_ISP_DNS_RECS_TARGET."\"); $('#comment_val').html('.'); } } ");
	...
	function rebuildDNS($type) {
		...
		$services=""; //default
		if ($vhost['webserver_id']==0) $services="DNS,CP,MAIL,DKIM";
		if ($vhost['mailserver_id']==0) $services="DNS,CP,KMS,MAILING,FTP";
		$dns_zone_id=$this->_rebuildDNSzone($vhost['name'],$dns_zone['email'],$servers,$services,$dblink_cp,$dblink_erp);
		return $dns_zone_id;
	}

        function rebuildDKIM($type) {
                if (!isset($this->dbi)) $this->dbi = new dataDBI();
                include "shared/db_links.php";
                $this->trace("Rebuilding DNS zones...");
                if ($_GET['xid']=="") $this->_error("","xid parameter is missing.","fatal");
       ...
       function _setupMasterNameserver($domain,$nameserver) {
		...
		$command = "ssh -i /root/.ssh/id_rsa root@".$nameserver." \"sed '/".$domain."/,+8 d' /etc/named.conf > /tmp/named.conf\"  >> /var/log/kms.log";
		$this->ssh_exec("localhost",$command,"JADF7320cSJdcj3750492x42dj244");
                //copia registre 
                $command = "ssh -i /root/.ssh/id_rsa root@{$nameserver} 'cp /tmp/named.conf /etc/named.conf' >> /var/log/kms.log";
                $this->ssh_exec("localhost",$command,"JADF7320cSJdcj3750492x42dj244");
		//preparem registre de domini
                $command = "scp -i /root/.ssh/id_rsa /tmp/named_".$domain."* root@{$nameserver}:/tmp/";
                $this->ssh_exec("localhost",$command,"JADF7320cSJdcj3750492x42dj244");
		//afegeix registre a named
                $command = "ssh -i /root/.ssh/id_rsa root@{$nameserver} 'cat /tmp/named_".$domain.".tmp >> /etc/named.conf' >> /var/log/kms.log";
                $this->ssh_exec("localhost",$command,"JADF7320cSJdcj3750492x42dj244");
...
//               $command="ssh -i /root/.ssh/id_rsa root@$nameserver '/usr/local/kms/mod/isp/setup/create_dns_zone.sh ".$domain."'";
		...
		//elimina registre a named (si existeix)
                $command = "ssh -i /root/.ssh/id_rsa root@{$nameserver} \"sed '/".$domain."/,+0 d' /etc/named.conf > /tmp/named.conf\" >> /var/log/kms.log";
                $this->ssh_exec("localhost",$command,"JADF7320cSJdcj3750492x42dj244");
		//copia registre 
                $command = "ssh -i /root/.ssh/id_rsa root@{$nameserver} 'cp /tmp/named.conf /etc/named.conf' >> /var/log/kms.log";
                $this->ssh_exec("localhost",$command,"JADF7320cSJdcj3750492x42dj244");
		//preparem registre de domini
                ...
                $this->ssh_exec("localhost",$command,"JADF7320cSJdcj3750492x42dj244");
                //afegeix registre a named
                $command = "ssh -i /root/.ssh/id_rsa root@{$nameserver} 'cat /tmp/named2_".$domain.".tmp >> /etc/named.conf'  >> /var/log/kms.log";
                $this->ssh_exec("localhost",$command,"JADF7320cSJdcj3750492x42dj244");
		// copia slave zone
		$command = "scp -i /root/.ssh/id_rsa /tmp/".$domain." root@{$nameserver}:/var/named/slaves/";
		$this->ssh_exec("localhost",$command,"JADF7320cSJdcj3750492x42dj244");
		// chown named:named
		$command = "ssh -i /root/.ssh/id_rsa root@{$nameserver} 'chown named:named /var/named/slaves/".$domain."'  >> /var/log/kms.log";
                $this->ssh_exec("localhost",$command,"JADF7320cSJdcj3750492x42dj244");
		// named restart
	...
	function _setupNameservers ($domain,$servers) {
        ...
        function _reloadNamed($host) {
                        $this->ssh_exec("localhost","ssh -i /root/.ssh/id_rsa root@{$host} 'service named reload' >> /var/log/kms.log","JADF7320cSJdcj3750492x42dj244");
        }

        function _restartNamed($host) {
                        $this->ssh_exec("localhost","ssh -i /root/.ssh/id_rsa root@{$host} 'service named restart'  >> /var/log/kms.log","JADF7320cSJdcj3750492x42dj244");
        }
        ...
        function _setupPTR($domain,$nameserver) {
                ...
                $command = "ssh -i /root/.ssh/id_rsa root@{$nameserver} 'cat /tmp/named_".$domain."_reverse.tmp >> /var/named/master/".$file."'  >> /var/log/kms.log";
                $this->ssh_exec("localhost",$command,"JADF7320cSJdcj3750492x42dj244");
        }

	function _rebuildDNSzone($domain,$admin_email,$servers,$services,$dblink1,$dblink2) {
			include "shared/db_links.php";  // include_once no serveix
			if ($domain=="") $this->_error("","Missing 'domain' parameter","fatal");
			if (!isset($this->dbi)) $this->dbi = new dataDBI();
			...
			$dns_zone_id=$this->setupDNSzone($domain,$admin_email,$servers,$services,$dblink1,$dblink2);
			return $dns_zone_id;
	}

        function setupDNSzone($domain,$admin_email,$servers,$services,$dblink1,$dblink2) {
			if (!isset($this->dbi)) $this->dbi = new dataDBI();
			include "shared/db_links.php";
...
//				$this->trace("DNS zone is missing. Creating it...");
				$this->trace("Setting up DNS zone...");
                                ...
                                $values=array("status"=>1,"name"=>$domain,"email"=>$admin_email,"type"=>"master","ttl"=>86400,"ttl_unit"=>86400,"refresh"=>10800,"refresh_unit"=>3600,"retry_unit"=>3600,"expire"=>604800,"expire_unit"=>86400,"minimum"=>10800,"minimum_unit"=>3600,"serial_format"=>"UNIXTIMESTAMP","serial"=>date('Ymds'));
				...
				if ($services=="") $services="DNS,WEB,DB,MAIL,MAILING,CP,FTP,KMS";
				...
				$this->dbi->update_record("kms_isp_dns_zones",$data,"id=$dns_zone_id",$dblink_cp,$dblink_erp);
                        ...
                        $res2=mysqli_query($this->dblinks['client'],$select);
                                while ($subdomain=mysqli_fetch_assoc($res2)) {
			...
			$select="select * from kms_isp_dns_recs where dns_zone_id={$dns_zone_id} order by type desc,host";
				...
				if (strpos($rec['val'],"DKIM")&&strlen($rec['val'])>251) {
						...
						$new_val=substr($new_val,0,strlen($new_val)-2).")";
						$rec['val']=$new_val;
				}
...
//		if ($dblink_cp=="") include "shared/db_links.php";
		...
		$command = "ssh -i /root/.ssh/id_rsa root@".$servers['nameserver1']." \"sed '/".$vhost['domain']."/,+8 d' /etc/named.conf > /tmp/named.conf\"  >> /var/log/kms.log";
		$this->ssh_exec("localhost",$command,"JADF7320cSJdcj3750492x42dj244");
		$command = "ssh -i /root/.ssh/id_rsa root@".$servers['nameserver1']." 'mv /tmp/named.conf /etc/named.conf'  >> /var/log/kms.log";
		$this->ssh_exec("localhost",$command,"JADF7320cSJdcj3750492x42dj244");
		$command = "ssh -i /root/.ssh/id_rsa root@".$servers['nameserver1']." 'rm /var/named/master/".$vhost['domain']."'  >> /var/log/kms.log";
                $this->ssh_exec("localhost",$command,"JADF7320cSJdcj3750492x42dj244");
		$this->_reloadNamed($servers['nameserver1']);	
		
		//secundari
		$command = "ssh -i /root/.ssh/id_rsa root@".$servers['nameserver2']." \"sed '/".$vhost['domain']."/$ d' /etc/named.conf > /tmp/named.conf\"  >> /var/log/kms.log";
		$this->ssh_exec("localhost",$command,"JADF7320cSJdcj3750492x42dj244");
		$command = "ssh -i /root/.ssh/id_rsa root@".$servers['nameserver2']." 'mv /tmp/named.conf /etc/named.conf'  >> /var/log/kms.log";
                $this->ssh_exec("localhost",$command,"JADF7320cSJdcj3750492x42dj244");
		$command = "ssh -i /root/.ssh/id_rsa root@".$servers['nameserver2']." 'rm /var/named/slaves/".$vhost['domain']."'  >> /var/log/kms.log";
                $this->ssh_exec("localhost",$command,"JADF7320cSJdcj3750492x42dj244");
	...
	function getServers($vhost,$type) {
		...
		$servers=array();
                if ($type=="vhost") {
                        $servers["webserver"]=$this->dbi->get_record("select hostname,ip from kms_isp_servers where id=".$vhost['webserver_id'],$dblink_cp); 
                        $servers["mailserver"]=$this->dbi->get_record("select hostname,ip from kms_isp_servers where id=".$vhost['mailserver_id'],$dblink_cp); 
                        ...
                        $servers["webserver"]=$this->dbi->get_record("select hostname,ip from kms_isp_servers where id=".$vhost['webserver_id'],$dblink_cp); 
                        $servers["mailserver"]=$this->dbi->get_record("select hostname,ip from kms_isp_servers where id=".$vhost['mailserver_id'],$dblink_cp); // mx2
                        $servers["nameserver1"]=$this->dbi->get_record("select hostname,ip from kms_isp_servers where id=1",$dblink_cp); // ns3
                        $servers["nameserver2"]=$this->dbi->get_record("select hostname,ip from kms_isp_servers where id=8",$dblink_cp); // ns4
                }
                return $servers;
	...
	function onPreInsert ($post) {
		$vhost=$this->dbi->get_record("SELECT * from kms_isp_hostings_vhosts WHERE id='".$_GET['xid']."'");
		//format
		if ($post['type']=="NS"||$post['type']=="A"||$post['type']=="AAA"||$post['type']=="CNAME"||$post['type']=="MX"||$post['type']=="SRV") {
      ...
      function onPreUpdate ($post) {
		...
		$servers=$this->getInstallServers("vhost");
                $this->setupDNSzone($vhost['name'],$dns_zone['email'],$servers,"DNS,WEB,DB,MAIL,MAILING,CP,FTP,KMS",$dblink_cp,$dblink_erp);
        }
