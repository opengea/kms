...
class isp_clients extends mod {
	...
	var $key	= "id";	
	var $fields = array("sr_client","status","name","contacts","creation_date");
	...
	var $hidden = array("credit_card","credit_card_name","credit_card_expiration_date","credit_card_vcs","username","password");
        /*=[ PERMISSIONS ]===========================================================*/
	...
	//		$_GET['app']="cp-reseller";
			$this->notedit=array("comments","sr_provider","sr_client","dr_folder","bank_name","default_payment_day");
                        $this->where = "sr_client in (select sr_client from kms_isp_clients where sr_provider=".$client['sr_client'].")";
			if ($_GET['action']=='edit_client') { 
						$this->can_delete=false;
						$_GET['_']="e"; $_GET['id']=$client['id']; 
			} else { 
//						array_push($this->notedit,"billing_email"); 
				$this->fields = array("sr_client","status","name","email", "billing_email","creation_date");			
			}
...
//		$this->action("update_domains","/usr/local/kms/mod/isp/update_domains.php");
		...
		$this->setComponent("select","sr_payment_method",array("11"=>_KMS_ERP_PAYMENTM_CREDITCARD,"3"=>_KMS_ERP_PAYMENTM_DOMICILIACIO,"1"=>_KMS_ERP_PAYMENTM_TRANSFER,"16"=>"PayPal"));
  ...
  //              array_push($this->tabs,$tab);
	...
	function onInsert($post,$id) {
		...
		$select="SELECT * FROM kms_isp_clients WHERE sr_user=".$this->user_account['id'];
                $result=mysqli_query($dblink_cp,$select);
                $client=mysqli_fetch_array($result);
...
//                                $this->trace("username already exists, renaming...");
		...
		if ($post['type']=="reseller"&&$post['billing_client']=="1") $client_group=4; //user (no invoices)
...
//			$id_contact=$this->dbi->insert_record("kms_sys_users",$post,$dblink_erp)
		...
		$original_client=mysqli_fetch_array($result);	
		$select="select * from kms_ent_contacts WHERE id=".$client_extended['sr_client'];
                $result=$this->dbi->query($select,$dblink);
                $original_entity=mysqli_fetch_array($result);		
		...
		$update=$this->dbi->make_update($merge,"kms_ent_clients","sr_client=".$client_extended['sr_client'],$fields_to_update,$dblink);
		$this->dbi->query($update,$dblink);
		//update ent_contacts
		$fields_to_update=array("name","sector","contacts","alt_phone","web","address","location","province","zipcode","country","language","newsletter");
		$update=$this->dbi->make_update($post,"kms_ent_contacts","id=".$client_extended['sr_client'],$fields_to_update,$dblink);
		$this->dbi->query($update,$dblink);
        }
}
?>
