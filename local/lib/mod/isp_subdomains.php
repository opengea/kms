...
class isp_subdomains extends mod {
	...
	var $key	= "id";	
	var $fields = array("name","vr_login");
	var $readonly = array("ftp_id");
	var $orderby = "name";
	var $notedit=array("domain_id","ftp_id","php","perl","python","fastcgi","ssl","vhost_id","ftp_main","ftp_id");
	var $hidden=array("ftp_id");
	...
	var $can_export = false;
       ...
       //*=[ CONSTRUCTOR ]===========================================================*/
        ...
        function isp_subdomains($client_account,$user_account,$dm) {
...
//                if ($vhost['id']!=""&&$_GET['_']=="b") $this->where = "kms_isp_ftps.vhost_id=".$_GET['xid']; 
		...
		$vhost=$this->dbi->get_record("select * from kms_isp_hostings_vhosts where id=".$_GET['xid']);
                $isp_ftps=new isp_ftps($this->client_account,$this->user_account,$this->dm,1);
...
//exit;
		...
		if ($_GET['xid']=="") $this->_error("","'xid' param missing.","fatal");
                ...
                $subdomain=$this->dbi->get_record("select * from kms_isp_subdomains where id={$id}");// AND vhost_id=".$_GET['xid']);
		$this->trace(print_r($subdomain,1));        
        // delete ftps
		if ($subdomain['ftp_id']=="") $this->trace("WARNING ftp_id is empty, can't remove FTP account");
