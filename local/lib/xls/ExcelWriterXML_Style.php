...
class ExcelWriterXML_Style {
	...
	public function getStyleXML(){
		...
		if ($this->useBorder){
			...
			foreach($positions as $position => $pData){
				...
				$bWeight = isset($pData['Weight'])
					...
					: '';
				$borders .= '<Border ss:Position="'.$position.'" '.$bLinestyle.' '.$bColor.' '.$bWeight.'/>'."\n";
			}
			$borders .= '</Borders>'."\n";
		}
		...
		if ($this->useInterior){
			...
			$interior = '<Interior '.$interiorColor.' '.$interiorPattern.' '.$interiorPatternColor.'/>'."\n";
		}
		
		if (!empty($this->numberFormat)) {$numberFormat = '			<NumberFormat ss:Format="'.$this->numberFormat.'"/>'."\n";}
		else $numberFormat = '			<NumberFormat/>'."\n";
		
		$xml = '		<Style ss:ID="'.$this->id.'" '.$name.'>'."\n";
		if ($this->useAlignment) $xml .= '			<Alignment '.$valign.' '.$halign.' '.$rotate.' '.$shrinktofit.' '.$wraptext.' '.$verticaltext.'/>'."\n";
		if ($this->useBorder) $xml .= $borders;
		if ($this->useFont) $xml .= '			<Font '.$fontSize.' '.$fontColor.' '.$bold.' '.$italic.' '.$strikethrough.' '.$underline.' '.$shadow.' '.$outline.' '.$fontName.' '.$fontFamily.'/>'."\n";
