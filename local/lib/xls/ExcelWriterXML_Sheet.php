...
@param string $function The name of the function that was called
	...
	public function getSheetXML(){
		...
		$xml = '<Worksheet ss:Name="'.$this->id.'" '.$displayRightToLeft.'>'."\n";
		$xml .= '	<Table>'."\n";
		foreach($this->colWidth as $colIndex => $colWidth){
			  $xml .= '<Column ss:Index="'.$colIndex.'" ss:AutoFitWidth="0" ss:Width="'.$colWidth.'"/>'."\n";
		}
...
//			$xml .= '	<Row ss:Index="'.$row.'" '.$rowHeight.' >'."\n";
			$xml .= '       <Row>'."\n";
			foreach($rowData as $column => $cell){
				...
				if (empty($this->comments[$row][$column])) $comment = '';
					...
					$comment = '<Comment ss:Author="'.$this->comments[$row][$column]['author'].'">'."\n";
					$comment .= '<ss:Data xmlns="http://www.w3.org/TR/REC-html40">'."\n";
					$comment .= '<B><Font html:Face="Tahoma" x:CharSet="1" html:Size="8" html:Color="#000000">'.htmlspecialchars($this->comments[$row][$column]['author']).':</Font></B>'."\n";
					$comment .= '<Font html:Face="Tahoma" x:CharSet="1" html:Size="8" html:Color="#000000">'.htmlspecialchars($this->comments[$row][$column]['comment']).'</Font>'."\n";
					$comment .= '</ss:Data>'."\n";
					$comment .= '</Comment>'."\n";
				}
				$type = $cell['type'];
...
//				$xml .= cleanspaces('		<Cell '.$style.' ss:Index="'.$column.'" '.$URL.' '.$mergeCell.' '.$formula.'>')."\n";
				$xml .= cleanspaces('		<Cell '.$style.' '.$URL.' '.$mergeCell.' '.$formula.'>')."\n";
				$xml .= '			<Data ss:Type="'.$type.'">';
//				$xml .= $this->removeBS(htmlspecialchars($data));
				$xml .= $data;//utf8_encode(htmlspecialchars($data));
				$xml .= '</Data>'."\n";
				$xml .= $comment;
				$xml .= '		</Cell>'."\n";
			}
