...
@param string $function The name of the function that was called
	...
	*/
	...
	public function writeData($target = null){
		...
		if (!empty($this->docTitle)) $docTitle = '<Title>'.htmlspecialchars($this->docTitle).'</Title>'."\n";
		if (!empty($this->docSubject)) $docSubject = '<Subject>'.htmlspecialchars($this->docSubject).'</Subject>'."\n";
		if (!empty($this->docAuthor)) $docAuthor = '<Author>'.htmlspecialchars($this->docAuthor).'</Author>'."\n";
		if (!empty($this->docLastAuthor)) $docLastAuthor = '<LastAuthor>'.htmlspecialchars($this->docLastAuthor).'</LastAuthor>'."\n";
		if (!empty($this->docCreated)) $docCreated = '<Created>'.htmlspecialchars($this->docCreated).'</Created>'."\n";
		if (!empty($this->docManager)) $docManager = '<Manager>'.htmlspecialchars($this->docManager).'</Manager>'."\n";
		if (!empty($this->docCompany)) $docCompany = '<Company>'.htmlspecialchars($this->docCompany).'</Company>'."\n";
...
//		$xml = trim(str_replace("\n","",str_replace("\n","",$xml)));	
	...
	public function docLastAuthor($lastauthor = ''){$this->docLastAuthor = $lastauthor;}
