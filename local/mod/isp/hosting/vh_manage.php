...
function explain_feature($feature,$colors) {
        ...
        $sel="SELECT * FROM intergrid_kms_erp.kms_ecom_services inner join kms_ecom_products on kms_ecom_services.product_id=kms_ecom_products.id and kms_ecom_services.subfamily=3 and kms_ecom_services.family=2 order by kms_ecom_products.id asc";
...
function contract_buttons($n) {
        ...
        $sel="SELECT * FROM intergrid_kms_erp.kms_ecom_services where kms_ecom_services.subfamily=3 and kms_ecom_services.family=2 and status='active' order by id asc";
        $res=mysqli_query($sel);
        $n=0; $out="";
        while ($product=mysqli_fetch_array($res)) {
...
if ($_GET['id']!="") {
	...
	$sel="select * from kms_ecom_products where family=2 and subfamily=3 order by id";
	$res=mysqli_query($sel);
}
?>
...
<tr>
	...
	?></h2>
	<span style="line-height:150%;color:#444;font-size:12px"><?echo _KMS_ISP_SELECT_VSERVER_EXPLAIN?></span><br><br>
<br><br>
    </td>
    <td></td>
</tr>
...
<tr class="feature"><td class="f2"><?=_KMS_ISP_SERVERS_VCPUS?></td><?=explain_feature("cpu")?></tr>
...
<tr class="feature"><td class="f2"><?=_KMS_ISP_HOSTING_FEATURE_FLEXIBLE_HOSTING?></td><?=explain_all("OK",$n)?></tr>
