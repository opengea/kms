...
$select = "SELECT * from kms_sys_bookmarks where userid='".$_SESSION['user_id']."' order by sort_order";
				$result=mysqli_query($link_clientdb,$select);
				if (!$link_clientdb) die ("[KMS quick_menu] Can't connect to database");
				
				if (!$result) die ("[KMS quick_menu] Error reading database".mysqli_error());
