...
class isp_extranets extends mod {
	...
	var $readonly = array("username","expiration_date","creation_date","updated_date","authcode","domain_name");
	...
	var $can_duplicate = true;
       ...
       //*=[ CONSTRUCTOR ]===========================================================*/
        ...
        function isp_extranets($client_account,$user_account,$dm,$dblinks) { //,$silent
...
//		 $this->setComponent("ranxid","password");
		...
		$this->maxlength("dbuser","25");
		$uploadDate = date('Y-m-d');
		$this->defvalue("creation_date",$uploadDate);
		$this->default_content_type = "host";
		$this->default_php = "hosts.php";
		$this->setComponent("xcombo","client_id",array("xtable"=>"kms_ent_clients","xkey"=>"id","xfield"=>"username","readonly"=>false,"linkcreate"=>false,"linkedit"=>true,"sql"=>"select kms_ent_clients.id,kms_ent_contacts.name from kms_ent_clients inner join kms_ent_contacts on kms_ent_clients.sr_client=kms_ent_contacts.id"));
		 $this->setComponent("xcombo","hosting_id",array("xtable"=>"kms_isp_hostings","xkey"=>"id","xfield"=>"description","readonly"=>false,"linkcreate"=>false,"linkedit"=>true,"sql"=>""));
//		$this->setComponent("xcombo","contract_id",array("xtable"=>"kms_erp_contracts","xkey"=>"id","xfield"=>"description","readonly"=>false,"linkcreate"=>true,"linkedit"=>true,"sql"=>"select description from kms_erp_contracts where sr_client="));
		$this->setComponent("xcombo","contract_id",array("xtable"=>"kms_erp_contracts","xkey"=>"id","xfield"=>"domain","readonly"=>false,"linkcreate"=>false,"linkedit"=>true,"sql"=>"select kms_erp_contracts.id,CONCAT(domain,' - ',name_ca) from kms_erp_contracts INNER JOIN kms_ecom_services ON kms_erp_contracts.sr_ecom_service=kms_ecom_services.id and (family=3 or family=2) and kms_erp_contracts.status='active' order by domain"));
...
//		$this->setComponent("cipher","password","plain");
	 ...
	 function onInsert ($post,$id) {
		...
		$extended_client=mysqli_fetch_array($result);
