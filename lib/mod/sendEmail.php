...
require_once ('/usr/local/kms/lib/openExtranetDB.php');
...
$_table_mailing_enviats = array('id','serial','to','from','sr_body','total_users','send_datetime');
...
//-- Selecionamos la sentencia de dicho grupo:	
        $query = mysqli_query($this->dblinks['client'],"SELECT * FROM kms_mailing_groups WHERE name  = '$_grupo' LIMIT 1");
...
//-- Ejecutamos la sentencia que corresponde a dicho grupo:	
        $query = mysqli_query($this->dblinks['client'],$row['sql']);
...
//	$_table_mailing_enviats[6] = mysqli_num_rows($sql);
	$num_total = mysqli_num_rows($sql);
	...
	while($row = mysqli_fetch_assoc($sql))
	{
        ...
        $query = mysqli_query($this->dblinks['client'],"SELECT MAX(id) as last_id FROM kms_mailings");
        $row_last_id = mysqli_fetch_array($query);
...
*/
   $query = mysqli_query($this->dblinks['client'],"UPDATE kms_imark_mailings SET total_users = $num_total, send_datetime = '".date('Y-m-d')."',status='sent' WHERE id=$last_id");
