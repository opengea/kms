...
//	var $sum 	= array("base","total_tax","total");
	...
	var $can_duplicate = false; //* no duplicar perque es lien els ids
       ...
       function isp_invoices($client_account,$user_account,$dm) {
		include "/usr/local/kms/tpl/panels/isp_invoices.php";
			...
			$this->fields     = array("number", "creation_date", "sr_client", "concept", "total", "status", "payment_method","vr_payment_method", "sr_remittance","payment_date");
			...
			$_SESSION['where']=$this->where; //force see only invoices of this client on  /usr/local/kms/mod/isp/reports/report.php
		        $this->fields     = array("number", "creation_date", "concept", "base", "total_tax", "total", "payment_date", "payment_method","status");
                }
	...
	$this->abbreviate("payment_method",_KMS_ERP_PAYMENT_METHOD_PAYMENT_NAME);
...
//	$this->customOptions[0] = Array ("label"=>"_KMS_ERP_RPT_GENERATEPDF","url"=>"/kms/mod/erp/invoices/invoice.php?idfact=".array($this->fields['number'])."&pdf","ico"=>"pdf.gif","params"=>"action=view_pdf","target"=>"new","checkFunction"=>"");
	...
	$this->action("invoice_payment_stripe","/usr/local/kms/mod/isp/tpv/stripe_payment.php");
	...
	$xsql=array("xv_xtable"=>"kms_ent_clients", "xv_field"=>"sr_client", "xv_xfield"=>"sr_client", "xv_xselectionfield"=>"cif");
	$this->xvField("vr_cif",array("sql"=>$xsql));
	$xsql=array("xv_xtable"=>"kms_ecom_payment_methods", "xv_field"=>"payment_method", "xv_xfield"=>"id", "xv_xselectionfield"=>"payment_type");
