...
class sites_conf extends mod {

        /*=[ CONFIGURATION ]=====================================================*/

        var $table	= "kms_sites_conf";
	var $key	= "id";	
	var $fields = array( "name","value");
	...
	var $can_duplicate = true;
       ...
       //*=[ CONSTRUCTOR ]===========================================================*/
        ...
        function sites_conf ($client_account,$user_account,$dm) {
                parent::mod($client_account,$user_account,$extranet);
        }

        function setup($client_account,$user_account,$dm) {
                // set draggable
		$this->humanize("name","Field");
                $this->uid=$this->key;
                $this->rowclick = "drag"; //"edit"; //need $this->uid=$this-key; on setup(), and orderby="sort_order"
                $this->orderby="sort_order";

		if ($_GET['_']=="i") array_push($this->hidden,"value");
		else if ($_GET['_']=="e") { array_push($this->hidden,"type"); array_push($this->readonly,"name");  }
		$uploadDate = date('Y-m-d');
		$this->defvalue("creation_date",$uploadDate);
		$this->setComponent("select","type",array("integer"=>"integer","float"=>"float","relation"=>"relation","textfield"=>"textfield","date"=>"date","datetime"=>"datetime","boolean"=>"boolean","password"=>"password","color"=>"color","list"=>"list","openlist"=>"openlist","multipleselect"=>"multiple select open","multipleselect_rel"=>"multiple select related","text"=>"text","richtext"=>"rich text","html"=>"html","file"=>"file","image"=>"image","tags"=>"tags","multilang_textfield"=>"multilanguage textfield","multilang_richtext"=>"multilanguage richtext","multilang_html"=>"multilanguage html"));
                $select="select * from kms_sites_conf where id=".$_GET['id'];
		...
		switch ($conf['type']) {
			...
			case 'multiselect':
                        ...
                        case 'html':
                                ...
                                break;
                        case 'file':
                                $this->setComponent("file","value",array($this->kms_datapath."files/files","//data.".$this->current_domain."/files/files"));
                                break;
                        case 'image':
                                ...
                                $this->setComponent("picture","value",array("path"=>"/var/www/vhosts/".$this->current_domain."/subdomains/data/httpdocs".$path,"url"=>"//data.".$this->current_domain.$path,"resize_max_width"=>$field['size'],"resize_max_height"=>$field['size'],"thumb_max_width"=>100,"thumb_max_height"=>100,"scaleType"=>$options_arr['scaleType']));
                                break;
                        case 'tags':
