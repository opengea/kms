...
switch ($_GET['mod']) {
	...
	$result = mysqli_query($dblink_local"SELECT * FROM kms_ent_clients_extended WHERE id='".$document_data['sr_client']."'");
	if (!$result) {echo mysqli_error();exit;}
	$client_data = mysqli_fetch_array($result);
	$type = _KMS_ERP_INVOICE;
	break;
    case "erp_budgets":
    	$result = mysqli_query($dblink_local"SELECT * FROM kms_erp_budgets WHERE id='".$_GET['id']."'");
        if (!$result) {echo mysqli_error();exit;}
        $document_data = mysqli_fetch_array($result);
...
$result = mysqli_query($dblink_local"SELECT payment_name FROM kms_ecom_payment_methods where id='".$document_data['payment_method']."'");
if (!$result) {echo mysqli_error();exit;}
...
if (isset ($_GET['pdf']) || (isset ($_GET['sendmail']))) {
	...
	// introduim a la BD hora i email d'enviament
		$result = mysqli_query($dblink_local"UPDATE kms_".$_GET['mod']." SET sent_email='".$_GET['destiny']."',sent_date='".date('Y-m-d h:i:s')."',check_sent=1 WHERE id='".$_GET['id']."'");
		if (!$result) {echo mysqli_error();exit;}
		return true;
	} else  {
	        return false;
