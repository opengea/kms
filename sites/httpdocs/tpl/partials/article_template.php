...
if ($article['parent']=="") {
	...
	while ($child=mysqli_fetch_assoc($res_children)) { 
		$articlets[$i]=urlize($child['title_'.$_GET['l']]);
	        if ($_GET['a']!=""&&$_GET['a']==$articlets[$i]) $index=$i;
		$i++;
	} 

	$num_articlets=count($articlets);

} else {
	//article fill
	$child=mysqli_fetch_assoc($res_children);
	while ($child['id']!=$article['id']) { $articlets[$i]=urlize($child['title_'.$_GET['l']]);  $child=mysqli_fetch_assoc($res_children); $i++; }
	$child=mysqli_fetch_assoc($res_children);
	$articlets[$i]=urlize($child['title_'.$_GET['l']]); 
	$index=$i-1;
	$num_articlets=2;
